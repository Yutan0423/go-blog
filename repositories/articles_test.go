...
func TestSelectArticleDetail(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.testTitle, func(t *testing.T) {
			got, err := repositories.SelectArticleDetail(testDB, test.expected.ArticleID)
			if err != nil {
				t.Fatal(err)
			}

			if got.ArticleID != test.expected.ArticleID {
				t.Errorf("ArticleID: get %d but want %d\n", got.ArticleID, test.expected.ArticleID)
			}
...
func TestInsertArticle(t *testing.T) {
	...
	if newArticle.ArticleID != expectedArticleNum {
		t.Errorf("new article id is expected %d but got %d\n", expectedArticleNum, newArticle.ArticleID)
	}
	...
	t.Cleanup(func() {
		...
		`
		testDB.ExecContext(context.Background(), sqlStr, article.Title, article.Contents, article.UserName)
	})
}
